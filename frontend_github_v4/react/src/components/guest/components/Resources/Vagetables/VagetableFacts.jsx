...
export const ShowVagetableLoader = async({params}) =>{
    const {id} = params
    try {
       const {data} = await axiosClient.get('/guest/vagetable/show/'+id)
       return data
    } catch (error) {
        console.log(error)
        return error
...
export default function VagetableFacts(){

    const vagetables = useLoaderData()
                ...
                <div className="flex flex-row gap-5 shadow-lg flex-wrap w-full items-stretch justify-center">
                     <div className="w-full border-l-[5px] flex flex-col gap-5 p-5 text-lg border-green-700" style={{ background:'rgb(240 253 244)' }}>
                        ...
                        <ul className="list-disc flex flex-col items-start px-20">
                            {vagetables.facts && vagetables.facts.map((fact,index)=>(
                        ...
                        <ul className="list-disc flex flex-col items-start px-20">
                            {vagetables.benefits && vagetables.benefits.map((benefit,index)=>(
