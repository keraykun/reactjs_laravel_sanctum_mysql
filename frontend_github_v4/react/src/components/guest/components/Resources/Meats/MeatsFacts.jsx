...
export const ShowMeatsLoader = async({params}) =>{
    const {id} = params
    try {
       const {data} = await axiosClient.get('/guest/meat/show/'+id)
       return data
    } catch (error) {
        console.log(error)
        return error
...
export default function MeatsFacts(){
                ...
                <div className="flex flex-row gap-5 shadow-lg flex-wrap w-full items-stretch justify-center">
                     <div className="w-full border-l-[5px] flex flex-col gap-5 p-5 text-lg border-green-700" style={{ background:'rgb(240 253 244)' }}>
                        ...
                        <ul className="list-disc flex flex-col items-start px-20">
                            {meats.facts && meats.facts.map((fact,index)=>(
                        ...
                        <ul className="list-disc flex flex-col items-start px-20">
                            {meats.benefits && meats.benefits.map((benefit,index)=>(
