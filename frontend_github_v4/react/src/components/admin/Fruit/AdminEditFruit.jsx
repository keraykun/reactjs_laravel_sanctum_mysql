...
export const AdminEditFruitLoader = async({params}) =>{
    const {id} = params
    try {
       return axiosClient.get('admin/fruit/edit/'+id).
        then(({data})=>{
            return data
        })
    } catch (error) {
...
// export const AdminUpdateFruitAction = async({ request }) =>{
//     const formData = await request.formData();
//     const payload = {
//         id:formData.get('primary_id'),
//         name:formData.get('name'),
//         description:formData.get('description'),
//         image:'gogo',
//     }
//     return await axiosClient.post('admin/fruit/update/'+payload.id,payload).
//         then(({data})=>{
//             console.log(data)
//             return toast.fire({
//                 title: 'Updated!',
...
export default function AdminEditFruit(){
    ...
    const imageHandler = (event) =>{
      ...
      const handleSubmit = (e) => {
        ...
        formData.append('facts_delete',JSON.stringify(factsDelete))
         ...
         axiosClient.post('admin/fruit/update/'+data.id,formData).then((response=>{
    ...
    //         <form onSubmit={handleSubmit} method="post" className="bg-white shadow-md rounded min-w-[500px] px-8 pt-6 pb-8 mb-4 flex-1 max-w-lg" encType="multipart/form-data"  id="clear-form">
    ...
    //                 Fruit Name
    //             </label>
    //             <input name="name" value={name} onChange={(e)=>setName(e.target.value)} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"  type="text" placeholder="Enter fruit name"/>
    //             </div>
    //             <div className="mb-4">
    //             <label className="block text-gray-700 text-sm font-bold mb-2" >
    //                 Description
    //             </label>
    //             <Textarea name="description" rows={10} onChange={(e)=>setDescription(e.target.value)} defaultValue={description} label="Message"/>
    //             </div>
    //             <input name="primary_id" defaultValue={data.id} hidden />
    ...
    //            </button>
    //             }
    //             <Link to="/admin/fruits" className="bg-purple-500 hover:bg-purple-400 text-white py-2 rounded-md px-4">Back</Link>
    //             </div>
    //         </form>
    //        {avatar?
            ...
            <form  className="flex gap-10 max-w-8xl min-w-2xl bg-white shadow-md rounded w-full px-8 pt-6 pb-8 mb-4 flex-1" encType="multipart/form-data" onSubmit={handleSubmit} id="clear-form" method="post">
             <section className="w-[500px]">
                <div className="mb-4 relative">
                    ...
                    <Input name="name" value={name} onChange={(e)=>setName(e.target.value)} label="Fruit name" type="text" />
                    </div>
                    <div className="mb-4">
                    <label className="block text-gray-700 text-sm font-bold mb-2" >
                        Description
