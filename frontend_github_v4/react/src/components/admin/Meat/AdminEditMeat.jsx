...
export const AdminEditMeatLoader = async({params}) =>{
    const {id} = params
    try {
       return axiosClient.get('admin/meat/edit/'+id).
        then(({data})=>{
            return data
        })
    } catch (error) {
...
export default function AdminEditMeat(){
    ...
    const imageHandler = (event) =>{
      ...
      const handleSubmit = (e) => {
        ...
        formData.append('facts_delete',JSON.stringify(factsDelete))
         ...
         axiosClient.post('admin/meat/update/'+data.id,formData).then((response=>{
            return toast.fire({
            ...
            <form  className="flex gap-10 max-w-8xl min-w-2xl bg-white shadow-md rounded w-full px-8 pt-6 pb-8 mb-4 flex-1" encType="multipart/form-data" onSubmit={handleSubmit} id="clear-form" method="post">
             <section className="w-[500px]">
                <div className="mb-4 relative">
                    ...
                    <Input name="name" value={name} onChange={(e)=>setName(e.target.value)} label="Meat name" type="text" />
                    </div>
                    <div className="mb-4">
                    <label className="block text-gray-700 text-sm font-bold mb-2" >
                        Description
               ...
               <button className="bg-blue-200 cursor-not-allowed text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" disabled>
                ...
                <Link to="/admin/meats" className="bg-purple-500 hover:bg-purple-400 text-white py-2 rounded-md px-4">Back</Link>
                </div>
             </section>
             <section className="w-[500px]">
             <label className="block text-gray-700 text-sm font-bold mb-2" >
